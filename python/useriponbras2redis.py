...
def getUseripFromBras(brasName):
    ...
    dic_REGEXP = {
        "7750": '(Pool name\W+(pppoe)-\S+[\s\S]*?Totals for pool)|(Pool name\W+(game)\d{1,}[\s\S]*?Totals for pool)|(Pool name\W+(business)[\s\S]*?Totals for pool|Pool name\W+(vip10)[\s\S]*?Totals for pool)|(Pool name\W+(vip3)[\s\S]*?Totals for pool)|(Pool name\W+(vip1)[\s\S]*?Totals for pool)|(Pool name\W+(iptv)[\s\S]*?Totals for pool)',
        ...
        "me60": 'Pool-Name\s+: (business)[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: (game)\S+[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: (iptv)[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: (vip1)\S+[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: (vip3)[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: pppoe-(vip10)[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})|Pool-Name\s+: (pppoe)-user-pool\S+[\s\S]*?Gateway\s+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,}).+: (\d{1,}\.\d{1,}\.\d{1,}\.\d{1,})'
    }
